...
parser.add_argument('--model', action='store', choices=('default', 'defaultnorm1', 'defaultnorm0',),
...
from HEPCNN.size256.model_mydefault_single import MyModel
...
try:
    ...
    for epoch in range(args.epoch):
        ...
        for i, (data, label, weight, rescale, procIdx) in enumerate(tqdm(valLoader)):
            ...
            # print(pred.view(-1))
            # print(label.to('cpu'))
            # print(np.where(pred.to('cpu') > 0.5, 1, 0))
            val_loss += loss.item()
            val_acc += accuracy_score(label.to('cpu'), np.where(pred.to('cpu') > 0.5, 1, 0), sample_weight=weight.to('cpu'))
            matrix = matrix + confusion_matrix(label.to('cpu'), np.where(pred.to('cpu') > 0.5, 1, 0))
            # cut
