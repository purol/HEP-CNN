...
def getImage(energy, bins): # bins: [x axin bin num, y axis bin num]
    ...
    xlim = [0, 224] # range at x axis
    ylim = [0, 224] # range at y axis
...
class FileSplitOut:
    def __init__(self, maxEvent, args):
        ...
        if args.format == 'NHWC': ## TF default
            ...
            self.shape = [2, self.height, self.width]
            self.chAxis = 1
    ...
    def addEvents(self, src_weights, Energy_S, Energy_C):
        ...
        while begin < nSrcEvent:
            ...
            images_S = getImage(Energy_S[begin:end], [self.width, self.height])
            images_C = getImage(Energy_C[begin:end], [self.width, self.height])
            
            images_S = 1000 * images_S
            images_C = 1000 * images_C

            images_S = np.expand_dims(images_S, self.chAxis)
            images_C = np.expand_dims(images_C, self.chAxis)

            images = np.concatenate([images_S, images_C], axis=self.chAxis)
...
for nEvent0, srcFileName in zip(nEvent0s, srcFileNames):
    ...
    Energy_S = tree["Energy_S"].array()
    Energy_C = tree["Energy_C"].array()

    ## Save output
    fileOuts.addEvents(src_weights, Energy_S, Energy_C)
